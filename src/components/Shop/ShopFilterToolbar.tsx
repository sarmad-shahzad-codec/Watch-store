...
import type { ShopPriceFilter, ShopSortKey } from "./shopFilterHelpers";

const selectCn =
  "min-h-[40px] rounded-md border border-[#DDD5CC] bg-white px-3 py-2 text-[13px] text-[#2B1A0F] w-full focus:outline-none focus:ring-2 focus:ring-[#4A2F19]/20";
...
type Props = {
  ...
  onPriceChange: (value: ShopPriceFilter) => void;
  sort: ShopSortKey;
  onSortChange: (value: ShopSortKey) => void;
  productStyle: "grid" | "list";
  onProductStyleChange: (value: "grid" | "list") => void;
  resultCount: number;
  onClearFilters: () => void;
...
export const BRAND_OPTIONS: { value: string; label: string }[] = [
  ...
  { value: "Hublot Diamond", label: "Hublot Diamond" },
  { value: "Rolex", label: "Rolex" },
  { value: "Patek Philippe", label: "Patek Philippe" },
  { value: "Gloria Times", label: "Gloria Times" },
];

const PRICE_OPTIONS: { value: ShopPriceFilter; label: string }[] = [
  { value: "all", label: "Any price" },
  { value: "under_2500", label: "Under Rs. 2,500" },
  { value: "between_2500_3500", label: "Rs. 2,500 – Rs. 3,500" },
  { value: "between_3500_5000", label: "Rs. 3,500 – Rs. 5,000" },
  { value: "over_5000", label: "Rs. 5,000+" },
];

const SORT_OPTIONS: { value: ShopSortKey; label: string }[] = [
  { value: "latest", label: "Newest arrivals" },
  { value: "bestseller", label: "Top reviewed" },
  { value: "price_low", label: "Price: low → high" },
  { value: "price_high", label: "Price: high → low" },
  { value: "oldest", label: "Catalog order" },
];
...
const ShopFilterToolbar = ({
  ...
  return (
    <div className="rounded-xl bg-white border border-[#EDE4D8] shadow-[0_4px_24px_-8px_rgba(43,26,15,0.08)] mb-6 overflow-hidden">
      <div className="p-3 sm:p-4 md:p-5 flex flex-col gap-4">
        <div className="flex flex-col lg:flex-row lg:items-end gap-3 lg:gap-3 xl:gap-4">
          <div className="flex-1 min-w-0 w-full">
            ...
            <input
              ...
              className={`${selectCn} w-full lg:max-w-md`}
            />
          </div>

          <div className="grid grid-cols-1 xsm:grid-cols-2 lg:grid-cols-4 gap-3 flex-1">
            <div className="min-w-0">
              <label htmlFor="shop-brand" className="block text-[11px] font-medium uppercase tracking-wider text-[#6B5344] mb-1">
                Brand
              </label>
              <select
                id="shop-brand"
            ...
            <div className="min-w-0">
              <label htmlFor="shop-price" className="block text-[11px] font-medium uppercase tracking-wider text-[#6B5344] mb-1">
                Price
              </label>
              <select
                id="shop-price"
                value={price}
                onChange={(e) =>
                  onPriceChange(e.target.value as ShopPriceFilter)
                }
                className={`${selectCn} w-full`}
              >
                {PRICE_OPTIONS.map((o) => (
            ...
            <div className="min-w-0 xsm:col-span-2 lg:col-span-2">
              <label htmlFor="shop-sort" className="block text-[11px] font-medium uppercase tracking-wider text-[#6B5344] mb-1">
                Sort by
              </label>
              <select
                id="shop-sort"
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3 pt-1 border-t border-[#EDE4D8]/90">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
            <p className="text-custom-sm text-dark-3">
              Showing{" "}
              <span className="font-semibold text-[#2B1A0F]">{resultCount}</span>{" "}
              {resultCount === 1 ? "watch" : "watches"}
            </p>
            {hasActiveFilters ? (
              <button
                type="button"
                onClick={onClearFilters}
                className="text-[13px] font-medium text-[#8B5A2B] hover:text-[#4A2F19] underline-offset-2 hover:underline"
              >
                Clear filters
              </button>
            ) : null}
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2.5 shrink-0">
            <span className="text-[11px] uppercase tracking-wider text-dark-4 sm:hidden">
              Layout
            </span>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => onProductStyleChange("grid")}
                ...
                } flex items-center justify-center w-10 h-10 sm:w-10.5 sm:h-9 rounded-[5px] border ease-out duration-200 hover:bg-blue hover:border-blue hover:text-white`}
              >
                <svg
              ...
              <button
                type="button"
                onClick={() => onProductStyleChange("list")}
